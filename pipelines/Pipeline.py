...
@Author  : asanthan
@Descriptor: This is a Demonstration of Distributed RAG Pipeline to process any doc , any layout including multimodal LLM
"""
...
from ModelFactories import EmbedConnectorFactory, SinkConnectorFactory
from utils.connectors.SourceConnector import SourceConnector
...
class Pipeline(BaseModel):
  ...
  """
    ...
    trigger_schedule: Optional[TriggerSchedule] = Field(None,
                                                        description="Scheduling details for triggering the pipeline")
    ...
    def deserialize_embed(cls, value):
        if isinstance(value, dict):
            return EmbedConnectorFactory.EmbedConnectorFactory.get_embed(value.get("embed_name"), value.get("embed_information"))
        return value

    @validator("sink", pre=True, always=True)
    def deserialize_sink(cls, value):
        if isinstance(value, dict):
            return SinkConnectorFactory.SinkConnectorFactory.get_sink(value.get("sink_name"), value.get("sink_information"))
        return value
                    ...
                    for localFile in source.download_files(cloudFile=cloudFile):
                        for document in source.load_data(file=localFile):
                            for chunks in source.chunk_data(document=document):
                                ...
                                vectors_to_store = [
                                    NeumVector(id=str(uuid4()), vector=embeddings[i], metadata=chunks[i].metadata) for i
                                    in range(0, len(embeddings))]
