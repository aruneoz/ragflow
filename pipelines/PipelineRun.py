...
class PipelineRunStatus(BaseModel):
    ...
    def as_pipeline_run_status(detailed_status: dict):
        if detailed_status == None:
            return PipelineRunStatus(status="Unknown",
                                     message="Something went wrong in our end, please email kevin@tryneum.com and we will get you unblocked")
        return PipelineRunStatus(
...
class PipelineRun(BaseModel):
    ...
    def as_pipeline_run(dct: dict):
        if dct == None:
            return PipelineRun(id=None, created=None, pipeline_id=None, trigger_type=None, sync_type=None,
                               detailed_status=None, vectors_written=None, number_of_documents=None,
                               finished_distributing=None)
        return PipelineRun(
            id=dct.get("id", None),
            ...
            finished_distributing=dct.get("finished_distributing", None),
            # we could do something like in distributed tasks where we store the state of the DAG in the pipeline run object.. for now just doing finished_distributing
