...
if "system_prompt" not in st.session_state:
    st.session_state["system_prompt"] = "You are a helpful assistant with deep knowledge rockwell automation systems that answers questions based on the following context:{} . Analyze step by step and answer the question relevant to the context. If the context contains the Image Description section and user requests diagram or schematic , then return no response.  Limit your answers to the context provided."

with st.sidebar:
    st.title("Rockwell Chatbot")
...
if prompt := st.chat_input():
    ...
    json_response = pipeline.search(query=prompt, number_of_results=3)
    context=""
