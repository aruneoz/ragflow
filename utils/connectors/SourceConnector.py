...
from ModelFactories import ChunkerFactory, DataConnectorFactory, LoaderFactory
...
class SourceConnector(BaseModel):
    ...
    def chunk_data(self, document: NeumDocument) -> Generator[List[NeumDocument], None, None]:
        for chunk_set in self.chunker.chunk(documents=[document]):
            chunk_set_with_custom_metadata = [NeumDocument(id=chunk.id, content=chunk.content,
                                                           metadata={**chunk.metadata, **self.custom_metadata,
                                                                     **{"text": chunk.content}}) for chunk in chunk_set]
    ...
    def deserialize_data_connector(cls, value):
        if isinstance(value, dict):
            return DataConnectorFactory.DataConnectorFactory.get_data_connector(value.get("connector_name"),
                                                           value.get("connector_information"))
        return value

    @validator("chunker", pre=True, always=True)
    def deserialize_chunker(cls, value):
        if isinstance(value, dict):
            return ChunkerFactory.ChunkerFactory.get_chunker(value.get("chunker_name"), value.get("chunker_information"))
        return value

    @validator("loader", pre=True, always=True)
    def deserialize_loader(cls, value):
        if isinstance(value, dict):
            return LoaderFactory.LoaderFactory.get_loader(value.get("loader_name"), value.get("loader_information"))
        return value
